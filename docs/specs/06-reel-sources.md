...
- `ilfaaat_sistema`: пять папок — «Нейросети», «3д», «Нейросети для фото и видео»,
  «Повторить про нейросети», «Создание сайтов» (189 уникальных);
- `neyro_set7`: **все** папки (423);
- переписка с `neyro_set7`: метка «Директ» (815).

Итого 1364 уникальных рилса, значительная часть уже в базе (ТЗ 02 импортировало «директ + эти пять
папок» 06.09). Остальные 22 папки `ilfaaat_sistema` в этот заход не идут — скрипт их видит,
